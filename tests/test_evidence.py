...
def test_domain_inference_does_not_depend_on_filename(tmp_path):
    ...
    rows = [
        {
            "sample": i,
            "ph": 7 + i * 0.001,
            "turbidity": 0.2 + i * 0.01,
            "chlorine": 0.5 + i * 0.002,
        }
        for i in range(30)
    ]
    ...
    assert result.datasets[0].analysis_method == "IsolationForest"
...
def test_varying_file_count_and_order(tmp_path):
    ...
    water_rows = [
        {"ph": 7 + i / 1000, "conductivity": 500 + i}
        for i in range(25)
    ]
    energy_rows = [
        {
    ...
    assert {item.domain for item in result.datasets} == {
        "ENERGY / RESOURCE",
        "WATER QUALITY",
    }
...
def test_sha256_provenance_matches_file(tmp_path):
    path = tmp_path / "proof.csv"
    rows = [
        {"ph": 7 + i * 0.01, "turbidity": 0.1 + i * 0.01}
        for i in range(8)
    ]
    _write_csv(path, list(rows[0]), rows)

    item = analyze_package([path]).datasets[0]
...
def test_small_dataset_uses_robust_method(tmp_path):
    ...
    item = analyze_package([path]).datasets[0]
    assert item.analysis_method == "robust-MAD"
    assert item.analyzed_rows == 10


def test_too_few_records_do_not_invent_model_finding(tmp_path):
    path = tmp_path / "tiny.csv"
    rows = [
        {"ph": 7 + i * 0.1, "turbidity": 0.2 + i * 0.1}
        for i in range(4)
    ]
    _write_csv(path, list(rows[0]), rows)

    item = analyze_package([path]).datasets[0]
    assert item.analysis_method == "schema-only"
    assert item.anomaly_score == 0
    assert item.anomaly_flags == 0
...
def test_timestamp_inference_and_overlap(tmp_path):
    ...
    rows_a = [
        {
            "observed_at": f"2026-09-01T10:{i:02d}:00Z",
            "ph": 7 + i * 0.01,
            "turbidity": 0.2 + i * 0.01,
        }
        for i in range(10)
    ...
    rows_b = [
        {
            "event_time": f"2026-09-01T10:{i + 5:02d}:00Z",
            "flow": 100 + i,
            "pressure": 50 + i,
        }
        for i in range(10)
    ]
    _write_csv(first, list(rows_a[0]), rows_a)
    _write_csv(second, list(rows_b[0]), rows_b)

    result = analyze_package([first, second])
    assert result.datasets[0].timestamp_field == "observed_at"
    assert result.datasets[1].timestamp_field == "event_time"
...
def test_non_overlapping_time_windows_do_not_claim_correlation(tmp_path):
    ...
    rows_a = [
        {"timestamp": f"2026-09-01T10:{i:02d}:00Z", "ph": 7 + i * 0.01}
        for i in range(6)
    ]
    rows_b = [
        {
            "timestamp": f"2026-09-02T10:{i:02d}:00Z",
            "energy_kwh": 100 + i,
        }
        for i in range(6)
    ]
    ...
    assert analyze_package([first, second]).correlations == []


def test_report_contains_provenance_and_safety_language(tmp_path):
    path = tmp_path / "report.csv"
    rows = [
        {"ph": 7 + i * 0.01, "chlorine": 0.5 + i * 0.01}
        for i in range(8)
    ]
    _write_csv(path, list(rows[0]), rows)

    package = analyze_package([path])
    text = build_report(package)
    assert package.datasets[0].sha256 in text
    assert "not proof of contamination" in text
    assert "does not connect to or control PLCs" in text
